...
import (
	bech322 "github.com/zeta-chain/zetacore/precompiles/bech32"
	"math/big"
...
func (s *PrecompileTestSuite) TestNewPrecompile() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			s.SetupTest()
			p, err := bech322.NewPrecompile(tc.baseGas)
			if tc.expPass {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() *vm.Contract {
				...
				contract.Input = s.precompile.Methods[bech322.HexToBech32Method].ID
				return contract
			},
			func(data []byte) {},
			false,
		...
		},
		{
			...
			func(data []byte) {
				args, err := s.precompile.Unpack(bech322.HexToBech32Method, data)
		...
		},
		{
			...
			func(data []byte) {
				args, err := s.precompile.Unpack(bech322.Bech32ToHexMethod, data)
				s.Require().NoError(err, "failed to unpack output")
				s.Require().Len(args, 1)
