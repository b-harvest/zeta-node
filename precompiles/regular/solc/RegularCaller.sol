...
contract RegularCaller {
    function testBech32ToHexAddr() public view returns (bool) {
        ...
        string memory testBech32 = "zeta1h8duy2dltz9xz0qqhm5wvcnj02upy887fyn43u";
    ...
    function testBech32ify() public view returns (bool) {
        ...
        // Call the precompiled contract
        string memory result = IREGULAR_CONTRACT.bech32ify(testPrefix, testHexAddr);

        // Check if the result matches the expected output
        return keccak256(abi.encodePacked(result)) == keccak256(abi.encodePacked(expectedBech32));
    }

    function testRegularCall(string memory method, address addr) public returns (uint256) {
