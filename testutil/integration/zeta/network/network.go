...
import (
	...
	simapptypes "github.com/zeta-chain/zetacore/testutil/simapp"
	"math"
	"math/big"
	"time"
	...
	commonnetwork "github.com/zeta-chain/zetacore/testutil/integration/common/network"
)
...
var (
	...
	bondedAmt = sdktypes.TokensFromConsensusPower(1, ethermint.PowerReduction)
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new simapp with the following params
	simapp := createApp(n.cfg.chainID)

	// Configure Genesis state
	encCdc := app.MakeEncodingConfig()
	genesisState := app.NewDefaultGenesisState(encCdc.Codec)

	genesisState = setAuthGenesisState(simapp, genesisState, genAccounts)
	...
	genesisState = setStakingGenesisState(simapp, genesisState, stakingParams)

	//genesisState = setInflationGenesisState(simapp, genesisState)
	...
	genesisState = setBankGenesisState(simapp, genesisState, bankParams)
	...
	simapp.InitChain(
		abcitypes.RequestInitChain{
			...
			ConsensusParams: simapptypes.DefaultConsensusParams,
	...
	header := tmproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             simapp.LastBlockHeight() + 1,
		Time:               now,
	...
	simapp.BeginBlock(abcitypes.RequestBeginBlock{Header: header})

	// Set networks global parameters
	n.app = simapp
	// TODO - this might not be the best way to initilize the context
	n.ctx = simapp.BaseApp.NewContext(false, header)
	...
	simapp.BankKeeper.SetDenomMetaData(n.ctx, evmosMetadata)
