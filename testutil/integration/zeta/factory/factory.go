...
import (
	...
	commonfactory "github.com/zeta-chain/zetacore/testutil/integration/common/factory"
...
type TxFactory interface {
	...
	CallContractAndCheckLogs(privKey cryptotypes.PrivKey, txArgs evmtypes.TransactionArgs, callArgs CallArgs, logCheckArgs testutil.LogCheckArgs) (abcitypes.ResponseDeliverTx, *evmtypes.MsgEthereumTxResponse, error)
	// DeployContract deploys a contract with the provided private key,
	// compiled contract data and constructor arguments
	DeployContract(privKey cryptotypes.PrivKey, txArgs evmtypes.TransactionArgs, deploymentData ContractDeploymentData) (common.Address, error)
	// ExecuteContractCall executes a contract call with the provided private key
	ExecuteContractCall(privKey cryptotypes.PrivKey, txArgs evmtypes.TransactionArgs, callArgs CallArgs) (abcitypes.ResponseDeliverTx, error)
	...
	ExecuteEthTx(privKey cryptotypes.PrivKey, txArgs evmtypes.TransactionArgs) (abcitypes.ResponseDeliverTx, error)
	// EstimateGasLimit estimates the gas limit for a tx with the provided address and txArgs
	EstimateGasLimit(from *common.Address, txArgs *evmtypes.TransactionArgs) (uint64, error)
}
...
func (tf *IntegrationTxFactory) DeployContract(
	...
	txArgs.Input = (*hexutil.Bytes)(&data)
...
// ExecuteContractCall executes a contract call with the provided private key
func (tf *IntegrationTxFactory) ExecuteContractCall(privKey cryptotypes.PrivKey, txArgs evmtypes.TransactionArgs, callArgs CallArgs) (abcitypes.ResponseDeliverTx, error) {
	// Create MsgEthereumTx that calls the contract
	input, err := callArgs.ContractABI.Pack(callArgs.MethodName, callArgs.Args...)
	fmt.Println(" ExecuteContractCall call args", callArgs)
	fmt.Println(" ExecuteContractCall input", input, err)
	if err != nil {
		return abcitypes.ResponseDeliverTx{}, errorsmod.Wrap(err, "failed to pack contract arguments")
	}
	txArgs.Input = (*hexutil.Bytes)(&input)
...
// EstimateGasLimit estimates the gas limit for a tx with the provided address and txArgs
func (tf *IntegrationTxFactory) EstimateGasLimit(from *common.Address, txArgs *evmtypes.TransactionArgs) (uint64, error) {
...
func (tf *IntegrationTxFactory) populateEvmTxArgs(
	...
	txArgs evmtypes.TransactionArgs,
) (evmtypes.TransactionArgs, error) {
	tx := txArgs.ToTransaction().AsTransaction()
	if txArgs.ChainID == nil {
		ethChainID, err := ethermint.ParseChainID(tf.network.GetChainID())
		if err != nil {
			return evmtypes.TransactionArgs{}, errorsmod.Wrapf(err, "failed to parse chain id: %v", tf.network.GetChainID())
		}
		txArgs.ChainID = (*hexutil.Big)(ethChainID)
	}

	if tx.Nonce() == 0 {
		accountResp, err := tf.grpcHandler.GetEvmAccount(fromAddr)
		if err != nil {
			return evmtypes.TransactionArgs{}, errorsmod.Wrapf(err, "failed to get evm account: %s", fromAddr.String())
		}
		nonce := accountResp.GetNonce()
		txArgs.Nonce = (*hexutil.Uint64)(&nonce)
	}
	...
	if txArgs.GasPrice == nil {
		if txArgs.MaxFeePerGas == nil {
			txArgs.MaxPriorityFeePerGas = (*hexutil.Big)(big.NewInt(1))
		}
		if txArgs.MaxFeePerGas == nil {
			baseFeeResp, err := tf.grpcHandler.GetBaseFee()
			if err != nil {
				return evmtypes.TransactionArgs{}, errorsmod.Wrap(err, "failed to get base fee")
			}
			txArgs.MaxFeePerGas = (*hexutil.Big)(baseFeeResp.BaseFee.BigInt())
		}
	}
	...
	if txArgs.Gas == nil {
		...
		if err != nil {
			return evmtypes.TransactionArgs{}, errorsmod.Wrap(err, "failed to estimate gas limit")
		}
